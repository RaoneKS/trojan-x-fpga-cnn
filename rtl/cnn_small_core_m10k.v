...
module cnn_small_core (
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            P1R3: begin
                tval=c1_q;
                if(tval>pool_max) p1_wdata<=tval;
                else p1_wdata<=pool_max;
                state<=P1S;
            end
            P1S: begin
                ...
                if(kc==2) begin
                    ...
                    if(kr==2) begin
                        ...
                        if(in_ch==7) begin
                            ...
                            in_ch<=in_ch+1;
                            if(out_r>=0 && out_r<14 &&
                               out_c-1>=0 && out_c-1<14) begin
                                p1_raddr <= (in_ch+1)*196 +
                                            (out_r-1)*14 + (out_c-1);
                            end else begin
                                p1_raddr<=0;
                            end
                            c2w_ra <= out_f*72 + (in_ch+1)*9;
                            state<=C2W;
                        end
                    end else begin
                        kr<=kr+1;
            ...
            P2R3: begin
                tval=c2_q;
                if(tval>pool_max) p2_wdata<=tval;
                else p2_wdata<=pool_max;
                state<=P2S;
            end
            P2S: begin
            ...
            FCM: begin
                ...
                if(fc_feature==783) begin
                    ...
                    p2_raddr <= fc_feature+1;
                    fcw_ra <= class_idx*784 + fc_feature+1;
                    state<=FCW;
                end
            end
            FCS: begin
