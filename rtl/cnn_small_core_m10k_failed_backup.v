...
module cnn_small_core (
    ...
    localparam IDLE=5'd0,
               C1A=5'd1, C1W=5'd2, C1M=5'd3, C1S=5'd4,
               P1A=5'd5, P1W=5'd6, P1R0=5'd7, P1R1=5'd8, P1R2=5'd9, P1R3=5'd10, P1S=5'd11,
               C2A=5'd12, C2W=5'd13, C2M=5'd14, C2S=5'd15,
               P2A=5'd16, P2W=5'd17, P2R0=5'd18, P2R1=5'd19, P2R2=5'd20, P2R3=5'd21, P2S=5'd22,
               FCA=5'd23, FCW=5'd24, FCM=5'd25, FCS=5'd26,
               ARG=5'd27, FIN=5'd28;
    ...
    /* Defaults are assigned every cycle so the RAM write ports remain explicit. */
    always @(*) begin
        c1_we=1'b0; c1_addr=0; c1_wdata=0;
        p1_we=1'b0; p1_addr=0; p1_wdata=0;
        c2_we=1'b0; c2_addr=0; c2_wdata=0;
        p2_we=1'b0; p2_addr=0; p2_wdata=0;
    end
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            C1S: begin
                ...
                c1_raddr<=addr;
                /* Write happens through the RAM in the next combinational write cycle. */
                if(acc<0) c1_wdata<=0; else c1_wdata<=acc[31:0];
                c1_addr<=addr; c1_we<=1'b1;
                acc<=0;
             ...
             */
            P1A: begin
                p1_raddr <= pool_ch*784+(pool_r*2)*28+(pool_c*2);
            ...
            P1R0: begin
                ...
                p1_raddr<=pool_ch*784+(pool_r*2)*28+(pool_c*2)+1;
                state<=P1R1;
            end
            P1R1: begin
                tval=c1_q;
                if(tval>pool_max) pool_max<=tval;
                p1_raddr<=pool_ch*784+(pool_r*2+1)*28+(pool_c*2);
                state<=P1R2;
            end
            P1R2: begin
                tval=c1_q;
                if(tval>pool_max) pool_max<=tval;
                p1_raddr<=pool_ch*784+(pool_r*2+1)*28+(pool_c*2)+1;
                state<=P1R3;
            end
            P1R3: begin
                tval=c1_q;
                ...
                if(pool_c==13) begin
                    ...
                    if(pool_r==13) begin
                        ...
                        if(pool_ch==7) begin
                            ...
                            state<=C2A;
                        end else pool_ch<=pool_ch+1;
                    end else pool_r<=pool_r+1;
                end else pool_c<=pool_c+1;
            end
            ...
            C2S: begin
                ...
                c2_addr<=addr; c2_wdata<=(acc<0)?0:acc[31:0]; c2_we<=1'b1;
                acc<=0;
            ...
            P2S: begin
                ...
                p2_addr<=addr; p2_wdata<=pool_max; p2_we<=1'b1;
